...
本发布材料只更新 `https://tigao123.com/teaching-cpp/` 的静态前端，来源提交固定为 `9857842c4bf63eb23a27e295b7ea24186920b7fb`。它不迁移或连接数据库，不修改 `.env`、Apache 配置、主站 `.htaccess`、PM2 配置或 systemd 配置，不重启或平滑加载任何服务，也不操作 C++ 执行容器。
...
- 新前端继续使用 `teaching-prj-mgmt v0.1.3`，增加 C++ 自己的中文和英文界面设置；语言保存在独立的 `cpp:language` 项中，不读取或修改 p5.js 的语言设置，C++ 现有作品管理、拖放、编辑器、运行和保存行为保持不变。
...
- 当前公开前端必须与已成功发布的 `frontend-update-20260906-04` 四个构建文件完全一致。不同则只读预检停止，不猜测覆盖。
...
```powershell
& 'D:\Program Files\nodejs\node.exe' deploy\frontend-update-20260911-01\package.mjs
```
...
sha256sum -c teaching-cpp-frontend-update-20260911-01.tar.gz.sha256
test ! -e deploy/frontend-update-20260911-01
tar -tzf teaching-cpp-frontend-update-20260911-01.tar.gz
tar -xzf teaching-cpp-frontend-update-20260911-01.tar.gz
/var/www/teaching-cpp-backend/tools/node/bin/node deploy/frontend-update-20260911-01/update.mjs --check-only
```

压缩包内容必须全部位于 `deploy/frontend-update-20260911-01/`。`--check-only` 只核对包内清单、构建文件和可复现依赖，不读取生产配置或连接服务。
...
```bash
/var/www/teaching-cpp-backend/tools/node/bin/node deploy/frontend-update-20260911-01/update.mjs --preflight
```
