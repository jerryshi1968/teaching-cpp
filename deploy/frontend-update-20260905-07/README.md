...
本发布材料只更新 `https://tigao123.com/teaching-cpp/` 的静态前端，来源提交固定为 `ae7a5135f994ffa4f378cbb309ecb5c014d8cfb3`。它不迁移或连接数据库，不修改 `.env`、Apache 配置、主站 `.htaccess`、PM2 配置或 systemd 配置，不重启或平滑加载任何服务，也不操作 C++ 执行容器。
...
- 新前端在与 p5.js 作品管理页一致的全宽 `ProjectOrganizer` 作品工坊中分离卡片排序与移入作品组的拖放目标，阻止作品组拖入自身，并把作品与作品组卡片的操作按钮增大到与 p5.js 一致的尺寸。
...
- 当前公开前端必须与已成功发布的 `frontend-update-20260905-06` 四个构建文件完全一致。不同则只读预检停止，不猜测覆盖。
...
- `frontend-update-20260905-06` 已成功发布并通过公开检查，修正了卡片操作按钮的一致性；本版本只修正拖放排序与移入作品组的目标冲突，并增大卡片操作按钮。
...
```powershell
& 'D:\Program Files\nodejs\node.exe' deploy\frontend-update-20260905-07\package.mjs
```
...
sha256sum -c teaching-cpp-frontend-update-20260905-07.tar.gz.sha256
test ! -e deploy/frontend-update-20260905-07
tar -tzf teaching-cpp-frontend-update-20260905-07.tar.gz
tar -xzf teaching-cpp-frontend-update-20260905-07.tar.gz
/var/www/teaching-cpp-backend/tools/node/bin/node deploy/frontend-update-20260905-07/update.mjs --check-only
```

压缩包内容必须全部位于 `deploy/frontend-update-20260905-07/`。`--check-only` 只核对包内清单、构建文件和可复现依赖，不读取生产配置或连接服务。
...
```bash
/var/www/teaching-cpp-backend/tools/node/bin/node deploy/frontend-update-20260905-07/update.mjs --preflight
```
