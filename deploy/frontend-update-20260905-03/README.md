...
- 当前公开前端必须与提交 `5760336` 的已复现构建完全一致；该构建包含 2026-09-01 的前端品牌名称更新。不同则只读预检停止，不猜测覆盖。
...
```powershell
& 'D:\Program Files\nodejs\node.exe' deploy\frontend-update-20260905-03\package.mjs
```
...
sha256sum -c teaching-cpp-frontend-update-20260905-03.tar.gz.sha256
test ! -e deploy/frontend-update-20260905-03
tar -tzf teaching-cpp-frontend-update-20260905-03.tar.gz
tar -xzf teaching-cpp-frontend-update-20260905-03.tar.gz
/var/www/teaching-cpp-backend/tools/node/bin/node deploy/frontend-update-20260905-03/update.mjs --check-only
```

压缩包内容必须全部位于 `deploy/frontend-update-20260905-03/`。`--check-only` 只核对包内清单、构建文件和可复现依赖，不读取生产配置或连接服务。
...
```bash
/var/www/teaching-cpp-backend/tools/node/bin/node deploy/frontend-update-20260905-03/update.mjs --preflight
```
