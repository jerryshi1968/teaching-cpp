...
本发布材料只更新 `https://tigao123.com/teaching-cpp/` 的静态前端，来源提交固定为 `23e26caca276639ff8ae39994c2783437b7f91c2`。它不迁移或连接数据库，不修改 `.env`、Apache 配置、主站 `.htaccess`、PM2 配置或 systemd 配置，不重启或平滑加载任何服务，也不操作 C++ 执行容器。
...
- 新前端把作品与作品组卡片的操作图标固定为与 p5.js 一致的 `24×24`、按钮固定为 `36×36`，并在卡片底栏上方加入与 p5.js 一致的双色 2px 分隔线。
...
- 当前公开前端必须与已成功发布的 `frontend-update-20260905-07` 四个构建文件完全一致。不同则只读预检停止，不猜测覆盖。
...
- `frontend-update-20260905-07` 已成功发布并通过公开检查，分离了拖放排序与移入作品组的目标，并阻止作品组拖入自身；本版本只精确对齐操作图标、按钮盒子和底栏分隔线。
...
```powershell
& 'D:\Program Files\nodejs\node.exe' deploy\frontend-update-20260905-08\package.mjs
```
...
sha256sum -c teaching-cpp-frontend-update-20260905-08.tar.gz.sha256
test ! -e deploy/frontend-update-20260905-08
tar -tzf teaching-cpp-frontend-update-20260905-08.tar.gz
tar -xzf teaching-cpp-frontend-update-20260905-08.tar.gz
/var/www/teaching-cpp-backend/tools/node/bin/node deploy/frontend-update-20260905-08/update.mjs --check-only
```

压缩包内容必须全部位于 `deploy/frontend-update-20260905-08/`。`--check-only` 只核对包内清单、构建文件和可复现依赖，不读取生产配置或连接服务。
...
```bash
/var/www/teaching-cpp-backend/tools/node/bin/node deploy/frontend-update-20260905-08/update.mjs --preflight
```
