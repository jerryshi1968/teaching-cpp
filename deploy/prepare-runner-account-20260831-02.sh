...
// 只兼容已确认的 authselect 链接；保留链接与目标内容，其他文件仍拒绝链接。
export function nssSnapshot(io = fs) {
  const file = '/etc/nsswitch.conf';
  const allowedTarget = '/etc/authselect/nsswitch.conf';
  const secureDirectory = dir => {
    const stat = io.lstatSync(dir);
    ensure(stat.isDirectory() && !stat.isSymbolicLink() && io.realpathSync(dir) === dir && stat.uid === 0 && (stat.mode & 0o022) === 0, 'NSS_DIRECTORY_UNSAFE');
  };
  secureDirectory('/etc');
  const entry = io.lstatSync(file);
  ensure(entry.uid === 0 && entry.nlink === 1, 'NSS_ENTRY_UNSAFE');
  const linked = entry.isSymbolicLink();
  const linkText = linked ? io.readlinkSync(file) : null;
  const target = linked ? allowedTarget : file;
  if (linked) {
    ensure(path.posix.resolve('/etc', linkText) === allowedTarget, 'NSS_LINK_TARGET_UNEXPECTED');
    secureDirectory('/etc/authselect');
  }
  const stat = io.lstatSync(target);
  ensure(stat.isFile() && !stat.isSymbolicLink() && stat.nlink === 1 && stat.uid === 0 && (stat.mode & 0o022) === 0 && stat.size <= 262144, 'NSS_TARGET_UNSAFE');
  ensure(io.realpathSync(file) === target && io.realpathSync(target) === target, 'NSS_LINK_TARGET_UNEXPECTED');
  const content = io.readFileSync(target);
  const nss = content.toString('utf8').split('\n').map(line => line.split('#')[0].trim()).filter(line => /^subid\s*:/.test(line));
  ensure(nss.length === 0 || (nss.length === 1 && /^subid\s*:\s*files\s*$/.test(nss[0])), 'SUBID_SOURCE_NOT_FILES');
  return { file, target, linkText, entry: { uid: entry.uid, gid: entry.gid, mode: entry.mode & 0o7777 }, targetMode: stat.mode & 0o7777, targetUid: stat.uid, targetGid: stat.gid, sha256: createHash('sha256').update(content).digest('hex') };
}
export function assertNssUnchanged(before, io = fs) {
  ensure(JSON.stringify(nssSnapshot(io)) === JSON.stringify(before), 'NSS_CONFIGURATION_CHANGED');
}
...
async function prepare() {
  ...
  log('操作日志：' + logPath);
  step('只读预检：02 版兼容已确认的 authselect 链接');
  ...
  log('NSS 配置检查通过：' + (beforeNss.linkText === null ? beforeNss.file : beforeNss.file + ' -> ' + beforeNss.target) + '；不会修改此配置。');
  ...
  const unchangedPaths = [ROOT + '/.env', '/etc/selinux/config', '/etc/httpd/conf.d/tigao123-le-ssl.conf', '/etc/httpd/conf.d/tigao123.conf'];
  ...
  save('nss-before.json', beforeNss);
