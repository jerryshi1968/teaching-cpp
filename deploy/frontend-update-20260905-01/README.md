...
本发布材料只更新 `https://tigao123.com/teaching-cpp/` 的静态前端，来源提交固定为 `065eaefe3edf1903891d77580b9def3bafab6453`。它不迁移或连接数据库，不修改 `.env`、Apache 配置、主站 `.htaccess`、PM2 配置或 systemd 配置，不重启或平滑加载任何服务，也不操作 C++ 执行容器。
...
- 新前端包含 `ProjectOrganizer`、共享 CSS 和 C++ 适配器集成后的四个构建文件。
...
- 当前公开前端必须与 `web-publish-20260831-02/frontend-manifest.json` 完全一致。不同则只读预检停止，不猜测覆盖。
...
```powershell
& 'D:\Program Files\nodejs\node.exe' deploy\frontend-update-20260905-01\package.mjs
```
...
sha256sum -c teaching-cpp-frontend-update-20260905-01.tar.gz.sha256
test ! -e deploy/frontend-update-20260905-01
tar -tzf teaching-cpp-frontend-update-20260905-01.tar.gz
tar -xzf teaching-cpp-frontend-update-20260905-01.tar.gz
/var/www/teaching-cpp-backend/tools/node/bin/node deploy/frontend-update-20260905-01/update.mjs --check-only
```

压缩包内容必须全部位于 `deploy/frontend-update-20260905-01/`。`--check-only` 只核对包内清单、构建文件和可复现依赖，不读取生产配置或连接服务。
...
```bash
/var/www/teaching-cpp-backend/tools/node/bin/node deploy/frontend-update-20260905-01/update.mjs --preflight
```
