...
本发布材料只更新 `https://tigao123.com/teaching-cpp/` 的静态前端，来源提交固定为 `98fc87deb1157e0a2bc5542ffbb174544babcaae`。它不迁移或连接数据库，不修改 `.env`、Apache 配置、主站 `.htaccess`、PM2 配置或 systemd 配置，不重启或平滑加载任何服务，也不操作 C++ 执行容器。
...
- 新前端包含与 p5.js 作品管理页布局一致的全宽 `ProjectOrganizer` 作品工坊、共享 CSS、C++ 适配器，以及原 C++ 编辑、保存、运行、模板和课堂功能对应的四个构建文件。
...
- 当前公开前端必须与已成功发布的 `frontend-update-20260905-04` 四个构建文件完全一致。不同则只读预检停止，不猜测覆盖。
...
- `frontend-update-20260905-04` 已成功发布并通过公开检查，本版本在其基础上进一步对齐 p5.js 的班级选择、标题、按钮和作品卡片布局。
...
```powershell
& 'D:\Program Files\nodejs\node.exe' deploy\frontend-update-20260905-05\package.mjs
```
...
sha256sum -c teaching-cpp-frontend-update-20260905-05.tar.gz.sha256
test ! -e deploy/frontend-update-20260905-05
tar -tzf teaching-cpp-frontend-update-20260905-05.tar.gz
tar -xzf teaching-cpp-frontend-update-20260905-05.tar.gz
/var/www/teaching-cpp-backend/tools/node/bin/node deploy/frontend-update-20260905-05/update.mjs --check-only
```

压缩包内容必须全部位于 `deploy/frontend-update-20260905-05/`。`--check-only` 只核对包内清单、构建文件和可复现依赖，不读取生产配置或连接服务。
...
```bash
/var/www/teaching-cpp-backend/tools/node/bin/node deploy/frontend-update-20260905-05/update.mjs --preflight
```
