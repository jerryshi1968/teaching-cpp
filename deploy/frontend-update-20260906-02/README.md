# C++ 前端更新：teaching-prj-mgmt v0.1.2

本发布材料只更新 `https://tigao123.com/teaching-cpp/` 的静态前端，来源提交固定为 `85143273cb613900112164885f27040e3f2913e9`。它不迁移或连接数据库，不修改 `.env`、Apache 配置、主站 `.htaccess`、PM2 配置或 systemd 配置，不重启或平滑加载任何服务，也不操作 C++ 执行容器。
...
- 新前端升级到 `teaching-prj-mgmt v0.1.2`，修正作品及作品组向下拖到中间或末尾时恢复原位的问题，同时保留向上排序、跨组移动及面包屑拖放。
...
- 当前公开前端必须与已成功发布的 `frontend-update-20260906-01` 四个构建文件完全一致。不同则只读预检停止，不猜测覆盖。
...
```powershell
& 'D:\Program Files\nodejs\node.exe' deploy\frontend-update-20260906-02\package.mjs
```
...
sha256sum -c teaching-cpp-frontend-update-20260906-02.tar.gz.sha256
test ! -e deploy/frontend-update-20260906-02
tar -tzf teaching-cpp-frontend-update-20260906-02.tar.gz
tar -xzf teaching-cpp-frontend-update-20260906-02.tar.gz
/var/www/teaching-cpp-backend/tools/node/bin/node deploy/frontend-update-20260906-02/update.mjs --check-only
```

压缩包内容必须全部位于 `deploy/frontend-update-20260906-02/`。`--check-only` 只核对包内清单、构建文件和可复现依赖，不读取生产配置或连接服务。
...
```bash
/var/www/teaching-cpp-backend/tools/node/bin/node deploy/frontend-update-20260906-02/update.mjs --preflight
```
