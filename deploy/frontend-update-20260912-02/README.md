...
本发布材料只更新 `https://tigao123.com/teaching-cpp/` 的静态前端，来源提交固定为 `0bcd5ad4c190b4c63ceb05dac521d3e4731816ce`。它不迁移或连接数据库，不修改 `.env`、Apache 配置、主站 `.htaccess`、PM2 配置或 systemd 配置，不重启或平滑加载任何服务，也不操作 C++ 执行容器。
...
- 新前端继续使用 `teaching-prj-mgmt v0.1.3`，参照 p5.js 按登录用户保存当前班级和学生，并按作品所有者分别保存所在作品组；刷新页面后恢复有效状态，失效缓存安全回退，不改变现有作品管理、拖放、编辑器、运行和保存行为。
...
- 当前公开前端必须与服务器只读采集并固定在本版本中的四个构建文件完全一致。不同则只读预检停止，不猜测覆盖。
...
- `frontend-update-20260906-04` 已成功发布并通过公开检查，将作品组和作品分区显示。
- `frontend-update-20260911-01` 已成功发布并通过公开检查，增加了 C++ 独立的中文和英文界面设置。
- `frontend-update-20260912-01` 只执行过材料检查和只读预检；因服务器当前主脚本文件名及摘要与预期基线不同而停止，未建立锁、备份或切换前端，本版本以只读采集的实际基线取代它。
...
```powershell
& 'D:\Program Files\nodejs\node.exe' deploy\frontend-update-20260912-02\package.mjs
```
...
sha256sum -c teaching-cpp-frontend-update-20260912-02.tar.gz.sha256
test ! -e deploy/frontend-update-20260912-02
tar -tzf teaching-cpp-frontend-update-20260912-02.tar.gz
tar -xzf teaching-cpp-frontend-update-20260912-02.tar.gz
/var/www/teaching-cpp-backend/tools/node/bin/node deploy/frontend-update-20260912-02/update.mjs --check-only
```

压缩包内容必须全部位于 `deploy/frontend-update-20260912-02/`。`--check-only` 只核对包内清单、构建文件和可复现依赖，不读取生产配置或连接服务。
...
```bash
/var/www/teaching-cpp-backend/tools/node/bin/node deploy/frontend-update-20260912-02/update.mjs --preflight
```
