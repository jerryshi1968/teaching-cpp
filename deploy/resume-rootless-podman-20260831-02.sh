...
# 本文件只接续 rootless-init-X25XA6 在属性读取阶段的中断，不重做已完成的配置和管理器启动。
set -euo pipefail
umask 077
test "$#" -eq 0 || { printf '本脚本不接受参数。\n' >&2; exit 1; }
test "$(uname -s)" = Linux || { printf '只能在 Linux 服务器执行。\n' >&2; exit 1; }
test "$(id -u)" -eq 0 || { printf '请使用原来的 root 会话。\n' >&2; exit 1; }
exec /var/www/teaching-cpp-backend/tools/node/bin/node --input-type=module - --resume-rootless-podman <<'CPP_ROOTLESS_NODE'
...
const INIT_BACKUP = ROOT + '/backups/rootless-init-X25XA6';
...
export function parseProperties(text, fields = []) {
  ensure(typeof text === 'string' && text.trim().length > 0, 'PROPERTY_OUTPUT_EMPTY');
  const result = {};
  ...
  ensure(fields.every(field => Object.hasOwn(result, field)), 'PROPERTY_MISSING');
  return result;
}
// loginctl 252 的每个 --property 只接收一个属性名；重复参数，不把多个名字用逗号拼接。
export function propertyOptions(fields) {
  ensure(Array.isArray(fields) && fields.length > 0 && new Set(fields).size === fields.length && fields.every(field => typeof field === 'string' && /^[A-Za-z][A-Za-z0-9]*$/.test(field)), 'PROPERTY_NAMES_INVALID');
  return fields.map(field => '--property=' + field);
}
export function queryProperties(command, args, fields, read = run) {
  const output = read(command, [...args, ...propertyOptions(fields)]);
  try { return parseProperties(output, fields); }
  catch (error) {
    if (record) save('property-error-' + randomBytes(4).toString('hex') + '.json', { command, args, fields, output });
    error.propertySource = command + ':' + fields.join(',');
    throw error;
  }
}
function properties(unit, fields) { return queryProperties('systemctl', ['show', unit], fields); }
export function checkResumeRecord(before, created, savedPhase, laterFiles) {
  ensure(savedPhase === '只为 cpp-runner 启用后台用户管理，保持现有 slice 限额' && Array.isArray(laterFiles) && laterFiles.length === 0, 'RESUME_STAGE_UNEXPECTED');
  ensure(before.uid === UID && before.gid === GID && before.lingering === false && before.manager?.ActiveState === 'inactive' && before.manager?.Delegate === 'yes', 'RESUME_RECORD_UNEXPECTED');
  checkLimits(before.slice, 'inactive');
  ensure(created.storage === STORAGE_CONF && created.engine === ENGINE_CONF && /^[a-f0-9]{64}$/.test(created.storageSha256) && /^[a-f0-9]{64}$/.test(created.engineSha256), 'RESUME_CONFIG_RECORD_UNEXPECTED');
}
export function checkRecordedHashes(recorded, actual) {
  ensure(recorded && JSON.stringify(Object.keys(recorded).sort()) === JSON.stringify(Object.keys(actual).sort()), 'RESUME_CONFIG_SET_CHANGED');
  for (const [file, digest] of Object.entries(actual)) ensure(/^[a-f0-9]{64}$/.test(digest) && recorded[file] === digest, 'RESUME_EXISTING_CONFIGURATION_CHANGED');
}
...
async function initialize() {
  ...
  const logPath = ROOT + '/logs/rootless-resume-' + randomBytes(6).toString('hex') + '.log';
  logFd = fs.openSync(logPath, 'wx', 0o600);
  log('操作日志：' + logPath);
  step('只读接续检查：上次中断记录、已写入配置与用户管理器');
  ensure(hash(ROOT + '/deploy/initialize-rootless-podman-20260831.sh') === '718d5f2be70b8257bcb521c4d41046a3a6930d624537f9f5eed92986143e9c7a', 'INITIALIZATION_SCRIPT_CHANGED');
  directory(INIT_BACKUP, 0, 0o700);
  const failedBefore = readJson(INIT_BACKUP + '/before.json');
  const createdConfigs = readJson(INIT_BACKUP + '/created-configs.json');
  const phaseFile = INIT_BACKUP + '/phase.txt';
  const phaseStat = regular(phaseFile);
  ensure(phaseStat.uid === 0 && (phaseStat.mode & 0o077) === 0 && phaseStat.size < 2048, 'PRIVATE_RECORD_REQUIRED');
  const laterFiles = ['user-manager.json', 'probe-unit.json', 'podman-probe.json', 'result.json'].filter(name => exists(INIT_BACKUP + '/' + name));
  checkResumeRecord(failedBefore, createdConfigs, fs.readFileSync(phaseFile, 'utf8').trim(), laterFiles);
  ...
  for (const suffix of ['/runner-data', '/.config/systemd/user', '/.local/share', '/tmp']) ensure(fs.readdirSync(RUNNER_HOME + suffix).length === 0, 'RUNNER_DIRECTORY_NOT_EMPTY');
  ensure(JSON.stringify(fs.readdirSync(RUNNER_HOME + '/.config/containers').sort()) === JSON.stringify(['containers.conf', 'storage.conf']), 'RESUME_CONFIG_DIRECTORY_UNEXPECTED');
  for (const file of [RUNNER_HOME + '/.config/containers.conf', RUNNER_HOME + '/.config/environment.d', RUNNER_HOME + '/.config/containers/systemd', ROOT + '/.env.runner', RUNTIME + '/containers', RUNTIME + '/libpod', RUNTIME + '/podman/podman.sock']) ensure(!exists(file), 'PODMAN_ALREADY_TOUCHED');
  ensure(exists('/var/lib/systemd/linger/' + ACCOUNT), 'LINGER_NOT_READY');
  ...
  checkLimits(beforeLimits, 'active');
  const beforeManager = properties(MANAGER, ['ActiveState', 'Delegate', 'ControlGroup']);
  ensure(beforeManager.ActiveState === 'active' && beforeManager.Delegate === 'yes' && beforeManager.ControlGroup === MANAGER_CGROUP, 'USER_MANAGER_NOT_READY');
  ...
  checkRecordedHashes(failedBefore.hashes, unchanged);
  assertNssUnchanged(failedBefore.nss);
  const beforePm2 = pm2Snapshot();
  checkSites(checkSiteValues);
  record = fs.mkdtempSync(ROOT + '/backups/rootless-resume-');
  log('私有操作记录：' + record);
  save('before.json', { uid: UID, gid: GID, nss: beforeNss, hashes: unchanged, pm2: beforePm2, slice: beforeLimits, manager: beforeManager, lingering: true, resumedFrom: INIT_BACKUP });

  step('核对上次已写入的容器配置和临时目录，逐字保留，不覆盖');
  ...
  for (const [file, content] of [[STORAGE_CONF, storageText], [ENGINE_CONF, engineText]]) {
    const stat = regular(file);
    ensure(stat.uid === UID && stat.gid === GID && (stat.mode & 0o777) === 0o600 && fs.readFileSync(file, 'utf8') === content, 'RESUME_CONFIG_CHANGED');
  }
  directory(RUNNER_HOME + '/tmp', UID, 0o700);
  ensure(hash(STORAGE_CONF) === createdConfigs.storageSha256 && hash(ENGINE_CONF) === createdConfigs.engineSha256, 'RESUME_CONFIG_CHANGED');
  save('preserved-configs.json', createdConfigs);

  step('读取已启动的用户管理器和 linger 状态，不再次启动或启用');
  const manager = properties(MANAGER, ['ActiveState', 'SubState', 'Delegate', 'MainPID', 'ControlGroup']);
  ...
  const userStatus = queryProperties('loginctl', ['show-user', ACCOUNT], ['Linger', 'RuntimePath']);
  ...
  save('result.json', { stage: 'rootless-initialized-no-images', at: new Date().toISOString(), account: ACCOUNT, uid: UID, gid: GID, summary, values, pm2: afterPm2, lingering: true, runEnabled: false, resumedFrom: INIT_BACKUP });
  log('rootless Podman 接续初始化完成；保留原有用户管理器和容器配置，仅执行初始化检查及其辅助进程。');
  log('未下载镜像、未运行学生代码、未启动 C++ 执行服务；两个网站进程未重启。');
  log('真实容器挂载、隔离、编译及容器内限额仍待下一步验证。');
  log('私有操作记录：' + record + '；日志：' + logPath);
}

if (process.argv[2] === '--resume-rootless-podman') {
  try { await initialize(); }
    ...
    if (error.propertySource) log('状态读取位置：' + error.propertySource);
