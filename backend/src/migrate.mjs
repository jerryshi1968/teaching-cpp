...
export async function applyMigration(connection, sql) {
  ...
  if (names.has('cpp_schema_migrations')) {
    const [rows] = await connection.query("SELECT * FROM cpp_schema_migrations WHERE id='001_cpp'");
    if (rows[0]?.state === 'complete' && rows[0]?.checksum === checksum) return { applied: false, message: '迁移已完成，未重复执行' };
    throw new Error('发现未完成或内容不一致的迁移记录。MySQL DDL 不能整体回滚，请先人工核对或恢复测试库，不自动重试');
  }
  ...
  await connection.execute("INSERT INTO cpp_schema_migrations(id,checksum,state) VALUES ('001_cpp',?,'applying')", [checksum]);
  ...
  for (let index = 0; index < statements.length; index++) {
    ...
    await connection.execute("UPDATE cpp_schema_migrations SET completed_statements=? WHERE id='001_cpp'", [index + 1]);
  }
  await connection.query("UPDATE cpp_schema_migrations SET state='complete' WHERE id='001_cpp'");
  return { applied: true, statements: statements.length };
}

if (process.argv[1] && import.meta.url === (await import('node:url')).pathToFileURL(process.argv[1]).href) {
  const sql = await migrationPlan();
  if (!process.argv.includes('--apply')) {
    ...
    try { console.log(await applyMigration(connection, sql)); } finally { await connection.end(); }
  }
}
