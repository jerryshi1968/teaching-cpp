...
- 当前公开前端必须与已成功发布的 `frontend-update-20260911-01` 四个构建文件完全一致。不同则只读预检停止，不猜测覆盖。
...
```powershell
& 'D:\Program Files\nodejs\node.exe' deploy\frontend-update-20260912-01\package.mjs
```
...
sha256sum -c teaching-cpp-frontend-update-20260912-01.tar.gz.sha256
test ! -e deploy/frontend-update-20260912-01
tar -tzf teaching-cpp-frontend-update-20260912-01.tar.gz
tar -xzf teaching-cpp-frontend-update-20260912-01.tar.gz
/var/www/teaching-cpp-backend/tools/node/bin/node deploy/frontend-update-20260912-01/update.mjs --check-only
```

压缩包内容必须全部位于 `deploy/frontend-update-20260912-01/`。`--check-only` 只核对包内清单、构建文件和可复现依赖，不读取生产配置或连接服务。
...
```bash
/var/www/teaching-cpp-backend/tools/node/bin/node deploy/frontend-update-20260912-01/update.mjs --preflight
```
