...
import { createFile, deleteFile, fileOf, normalizeProjectSource, renameFile, sameSource, sourceOf, updateFile } from './project-source.mjs';
...
export default function App() {
  ...
  const openProject = useCallback(async projectId => {
    ...
    try {
      ...
      const normalized = { ...data, ...normalizeProjectSource(data) };
      projectRef.current = normalized; savedRef.current = normalized;
      setProject(normalized); setActiveFile(normalized.entrypoint); setDirty(false); setSaveState('saved'); setRuns(history);
  ...
  const addFile = useCallback(() => {
    const filePath = window.prompt(t('editor.newFilePrompt'), 'utils.cpp')?.trim();
    if (!filePath) return;
    if (projectRef.current.files.some(file => file.path.toLocaleLowerCase() === filePath.toLocaleLowerCase())) { toast(t('editor.fileExists'), 'error'); return; }
    edit(createFile(projectRef.current, filePath)); setActiveFile(filePath);
  }, [edit, t, toast]);
  const moveFile = useCallback(() => {
    const filePath = window.prompt(t('editor.renameFilePrompt'), activeFile)?.trim();
    if (!filePath || filePath === activeFile) return;
    if (projectRef.current.files.some(file => file.path.toLocaleLowerCase() === filePath.toLocaleLowerCase())) { toast(t('editor.fileExists'), 'error'); return; }
    if (activeFile === projectRef.current.entrypoint && !/\.(?:c|cc|cpp|cxx)$/i.test(filePath)) { toast(t('editor.entrypointSourceRequired'), 'error'); return; }
    edit(renameFile(projectRef.current, activeFile, filePath)); setActiveFile(filePath);
  }, [activeFile, edit, t, toast]);
  const removeFile = useCallback(() => {
    const current = projectRef.current;
    if (activeFile === current.entrypoint) { toast(t('editor.entrypointRequired'), 'error'); return; }
    if (current.files.length === 1 || !window.confirm(t('editor.deleteFileConfirm', { path: activeFile }))) return;
    const next = deleteFile(current, activeFile);
    edit(next); setActiveFile(next.entrypoint);
  }, [activeFile, edit, t, toast]);
  const downloadProject = useCallback(value => downloadCode(JSON.stringify(sourceOf(value), null, 2), `${value.name || 'cpp-project'}.cpp-project.json`), []);
  ...
  }, [project?.files, project?.entrypoint, project?.build, project?.stdin, project?.profileId, dirty, conflict, draft, config?.writesEnabled, saveState, save]);
  useEffect(() => {
  ...
  const openDiagnostic = item => {
    if (item.file && project.files.some(file => file.path === item.file)) setActiveFile(item.file);
    requestAnimationFrame(() => requestAnimationFrame(() => {
      const view = editorRef.current;
      if (!view) return;
      const line = view.state.doc.line(Math.min(item.line, view.state.doc.lines));
      view.dispatch({ selection: { anchor: Math.min(line.to, line.from + item.column - 1) }, scrollIntoView: true }); view.focus();
    }));
  };
  ...
  const currentFile = project ? fileOf(project, activeFile) : null;
...
function ActionModal({ modal, groups, classes, onClose, onSubmit, onDelete }) {
  ...
  const snapshotSource = modal.type === 'snapshot' ? normalizeProjectSource(modal.data) : null;
  const [snapshotPath, setSnapshotPath] = useState(snapshotSource?.entrypoint || 'main.cpp');
