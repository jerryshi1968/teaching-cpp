...
import { assertUnpublishedResponse } from './entry.mjs';
import { parseApacheState, waitUntilApacheReady } from './lifecycle.mjs';
import { responseEvidence, failureEvidence } from './diagnostics.mjs';
...
const VERSION = 'web-publish-20260831-02';
...
function run(command, arguments_, timeout = 30000) {
  try { return execFileSync(command, arguments_, { cwd: ROOT, env: cliEnv, timeout, maxBuffer: 8 * 1024 * 1024, stdio: ['ignore', 'pipe', 'pipe'] }); }
...
function https(resource) {
  ...
  assert.ok(status, 'HTTPS 响应的状态行无法识别：' + resource);
...
function mainHealthy() {
  const page = https('/index.html');
  assert.equal(page.status, 200, '主站首页没有返回 200');
  assert.match(page.header, /content-type:[ \t]*text\/html/i, '主站首页类型不正确');
  assert.deepEqual(page.body, read('/var/www/html/index.html'), '主站 HTTPS 首页与实际文件不一致');
  if (mainIndex) assert.deepEqual(page.body, mainIndex, '主站首页在发布过程中发生变化');
  for (const resource of ['/', '/__cpp_main_route_probe']) {
    const response = https(resource);
    assert.equal(response.status, 200, '主站原路径未返回 200：' + resource);
    assert.deepEqual(response.body, page.body, '主站原路径不再返回原首页：' + resource);
  }
  assert.deepEqual(read(MAIN_RULES), mainRules, '主站 .htaccess 发生变化');
  return page.body;
}
...
async function apacheReady(restoring = false) {
  await waitUntilApacheReady({
    readState: remaining => parseApacheState(run('systemctl', ['show', 'httpd.service', '--property=LoadState,ActiveState,SubState', '--no-pager'], Math.min(3000, remaining)).toString('utf8')),
    onState: state => console.log('Apache 状态：' + JSON.stringify(state)),
    isCancelled: () => !restoring && interrupted
  });
  console.log('Apache 已稳定就绪。');
}
...
async function publish() {
  ...
  for (const module of ['proxy_module', 'proxy_http_module', 'ssl_module', 'dir_module', 'headers_module', 'mime_module', 'authz_core_module', 'rewrite_module']) assert.ok(modules.includes(module), 'Apache 缺少所需模块：' + module);
  original = read(SSL);
  ...
  oldIndex = p5Healthy();
  mainRules = read(MAIN_RULES);
  mainIndex = mainHealthy();
  const entryState = assertUnpublishedResponse(https('/teaching-cpp/'), mainIndex);
  console.log(entryState === 'main-site-fallback'
    ? '确认：C++ 目录不存在，当前 HTTP 200 内容与主站首页逐字一致，是主站回退；允许继续首次发布。'
    : '确认：C++ 目录和网页入口尚未发布。');
  ...
  run('cp', ['--preserve=all', '--', MAIN_RULES, backup + '/main.htaccess']);
  ...
  console.log('已在主站原 /api 代理前加入 Include：' + INCLUDE);
  syntax();
  progress('Apache 语法通过，平滑加载配置并验证 HTTPS 路由');
  run('systemctl', ['reload', 'httpd']);
  progress('等待 Apache 平滑加载完成，最多等待 15 秒');
  await apacheReady();
  await delay(500);
  progress('Apache 已就绪，开始核验 HTTPS 的 C++ API 与前端');
  ...
  assert.equal(settings.mode, 'production', '公开 C++ 配置不是生产模式'); assert.equal(settings.writesEnabled, false, '公开 C++ 配置的写入未关闭'); assert.equal(settings.runEnabled, false, '公开 C++ 配置的执行未关闭');
  ...
  for (const file of files) {
    ...
    assert.match(response.header, /x-content-type-options:[ \t]*nosniff/i, '前端资源缺少 nosniff 响应头：' + file.path);
  }
  ...
  assert.match(page.header, /content-security-policy:[^\r\n]*frame-ancestors 'none'/i, 'C++ 首页缺少预期的 CSP 响应头');
  const route = https('/teaching-cpp/__route_probe');
  assert.equal(route.status, 200, 'C++ 深路径没有返回前端页面'); assert.equal(sha256(route.body), indexHash, 'C++ 深路径内容不是 C++ 首页');
  ...
  for (const resource of ['/teaching-cpp/probe.env', '/teaching-cpp/probe.php']) {
    assert.ok([403, 404].includes(https(resource).status), '敏感或服务端文件路径没有被拒绝：' + resource);
  }
  p5Healthy(); mainHealthy(); await backend(); inspectFront(FRONT);
  assert.deepEqual(read(SSL), candidate);
  assert.deepEqual(read(HTTP), read(backup + '/tigao123.conf'));
  record('state.json', { stage: 'web-published-readonly', version: VERSION, pm2Saved, writesEnabled: false, runEnabled: false, mainHtaccessChanged: false, mainHtaccessSha256: sha256(mainRules), pm2: processBaseline, completedAt: new Date().toISOString() });
  ...
  console.log('主站首页和原路径回退正常，主站 .htaccess 原文件及其注释逐字未改。');
...
try {
  ...
  const failure = { stage, ...failureEvidence(error), lastRequest, pm2Saved };
  console.error('网页接入未完成；诊断：' + JSON.stringify(failure));
  if (error instanceof assert.AssertionError && !error.generatedMessage) console.error('检查说明：' + error.message);
  if (backup) record('failure.json', failure);
  if (apacheChanged || frontPublished) {
    try {
      if (apacheChanged) { replaceApache(original, candidate); syntax(); run('systemctl', ['reload', 'httpd']); await apacheReady(true); }
      ...
      console.error('恢复检查诊断：' + JSON.stringify(failureEvidence(restoreError)));
      console.error('无法确认自动恢复完成，请立即发回日志；不要重跑、删除目录或重启其他服务。');
    }
  }
