...
const materialNames = ['README.md', 'backend-manifest.json', 'guard.mjs', 'MATERIALS.sha256', 'package.mjs', 'update.mjs'];
...
assert.equal(run('git', ['rev-parse', `${SOURCE_COMMIT}^{commit}`]), SOURCE_COMMIT, '找不到已验证的后端来源提交');
run('git', ['merge-base', '--is-ancestor', SOURCE_COMMIT, 'HEAD']);
run('git', ['merge-base', '--is-ancestor', SOURCE_COMMIT, 'origin/main']);
run('git', ['diff', '--quiet', SOURCE_COMMIT, '--', 'backend/src/app.mjs', 'backend/src/service.mjs']);
for (const file of materials.manifest.files) run('git', ['ls-files', '--error-unmatch', file.path]);
...
const stage = await fs.mkdtemp(path.join(releases, '.backend-organizer-update-'));
const listFile = path.join(releases, `.backend-organizer-update-files-${randomUUID()}.txt`);
...
try {
  ...
  for (const file of materials.manifest.files) {
  ...
  if (path.dirname(realStage) !== realReleases || !path.basename(realStage).startsWith('.backend-organizer-update-')) throw new Error('暂存目录安全检查失败，不自动删除');
