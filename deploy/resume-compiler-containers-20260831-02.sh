...
# 02 接续版：保留旧失败现场，仅在新构建上下文中提前创建 WORKDIR，权限限制不变。
set -euo pipefail
umask 077
test "$#" -eq 0 || { printf '本脚本不接受参数。\n' >&2; exit 1; }
test "$(uname -s)" = Linux && test "$(id -u)" -eq 0 || { printf '请在 Linux 服务器原来的 root 会话执行。\n' >&2; exit 1; }
exec /var/www/teaching-cpp-backend/tools/node/bin/node --input-type=module - --resume-compiler-containers-02 <<'CPP_COMPILER_CHECK_NODE'
...
const LOCK = ROOT + '/backups/compiler-check-02.lock';
const FAILED_RECORD = ROOT + '/backups/compiler-check-FEQ0jn';
const FAILED_AREA = HOME_DIR + '/compiler-check-1d2b8fc0e133';
const FAILED_UNIT = 'cpp-compiler-check-1d2b8fc0e133.service';
const ORIGINAL_SCRIPT_SHA = '7619e68b594636617cee2f21a6623ff736ea9ecdf033abc55eba70690621ca1d';
...
let record, logFd, lockFd, old, imageHelpers, accountHelpers, repairHelpers, rootlessHelpers, diagnosticHelpers;
let phase = '初始检查';
...
export function buildContainerfile(original) {
  const text = Buffer.isBuffer(original) ? original.toString('utf8') : original;
  ensure(typeof text === 'string' && createHash('sha256').update(text).digest('hex') === '89400093593b873846787d97d59ae01f569a18190145b605d05b46a2dc2d26a8', 'ORIGINAL_CONTAINERFILE_CHANGED');
  const before = 'RUN test -x /usr/local/bin/g++ && mkdir -p /work && chmod 755 /work\nWORKDIR /work\n';
  const after = 'WORKDIR /work\nRUN test -x /usr/local/bin/g++ && test -d /work && stat -c "workdir=%u:%g:%a" /work && test "$(stat -c %a /work)" = 755\n';
  ensure(text.split(before).length === 2, 'CONTAINERFILE_PATCH_AMBIGUOUS');
  // 只变动构建上下文副本的两条指令；原 Containerfile、所有注释和其余原文保留。
  return Buffer.from(text.replace(before, after), 'utf8');
}
export function checkPreviousFailure(value) {
  ensure(value?.status === 1 && !value.signal && typeof value.stdout === 'string', 'PREVIOUS_RESULT_INVALID');
  const events = value.stdout.trim().split('\n').filter(Boolean).map(line => JSON.parse(line));
  const builds = events.filter(row => row.event === 'build-result');
  const errors = events.filter(row => row.event === 'error');
  const containers = events.filter(row => row.event === 'remaining-containers');
  ensure(builds.length === 1 && builds[0].result?.code === 1 && !builds[0].result.reason && !builds[0].result.signal, 'PREVIOUS_BUILD_NOT_EXPECTED');
  ensure(typeof builds[0].result.stderr === 'string' && builds[0].result.stderr.includes("mkdir: cannot create directory '/work': Permission denied"), 'PREVIOUS_BUILD_REASON_CHANGED');
  ensure(errors.length === 1 && errors[0].code === 'IMAGE_BUILD_FAILED' && !events.some(row => ['pass', 'complete', 'case-result', 'container-state-unconfirmed'].includes(row.event)), 'PREVIOUS_STAGE_CHANGED');
  ensure(containers.length === 1 && Array.isArray(containers[0].value) && containers[0].value.length === 0, 'PREVIOUS_CONTAINER_CLEANUP_UNCONFIRMED');
}
...
async function compilerWorker(unit, label, pin, diffIds, hashes) {
  ...
  try {
    ...
    const containerfile = buildContainerfile(checkSource(root + '/runner/Containerfile', hashes['runner/Containerfile']));
    fs.writeFileSync(area + '/build/Containerfile', containerfile, { flag: 'wx', mode: 0o600 });
    const containerfileSha256 = createHash('sha256').update(containerfile).digest('hex');
    emit({ event: 'context-ready', originalSha256: hashes['runner/Containerfile'], containerfileSha256 });
    ...
    emit({ event: 'complete', image, inspect, info, checks: checkCount, area, containerfileSha256, runEnabled: false, containers: 0 });
  } catch (error) {
...
export function workerSource(unit, label, pin, diffIds, hostChecker, spaceChecker, loadedChecker) {
  ...
  return '(async()=>{const fs=await import("node:fs");const {createHash}=await import("node:crypto");const IMAGE_LABEL=' + JSON.stringify(IMAGE_LABEL) + ';const CASES=' + JSON.stringify(CASES) + ';const ISOLATION_PROBE=' + JSON.stringify(ISOLATION_PROBE) + ';\n' +
    [ensure, safeCode, checkSource, buildContainerfile, checkTeachingImage, normalizeImageId, buildArguments, checkIsolation, checkCase, hostChecker, spaceChecker, loadedChecker].map(fn => fn.toString()).join('\n') + '\nawait (' + compilerWorker.toString() + ')(' + [unit, label, pin, diffIds, SOURCE_HASHES].map(value => JSON.stringify(value)).join(',') + ');})().catch(()=>{process.stderr.write("WORKER_BOOTSTRAP_FAILED\\n");process.exitCode=1;});';
}
...
async function loadChecks() {
  ...
  diagnosticHelpers = await old.loadHelpers(ROOT + '/deploy/diagnose-compiler-build-20260831.sh', '872a1c60a7f1d5d839d0661fc86cf7e5f28134dc760707b3887d3ca9b2f4e065', 'CPP_BUILD_DIAG_NODE');
  old.bindChecks(rootlessHelpers, accountHelpers, repairHelpers, record, logFd);
}
function previousFailure() {
  old.directory(FAILED_RECORD, 0, 0o700); old.directory(FAILED_AREA, 994, 0o700); old.directory(FAILED_AREA + '/build', 994, 0o700);
  checkSource(ROOT + '/deploy/verify-compiler-containers-20260831.sh', ORIGINAL_SCRIPT_SHA);
  const lock = old.readJson(ROOT + '/backups/compiler-check.lock');
  ensure(lock.record === FAILED_RECORD && Number.isSafeInteger(lock.pid) && lock.pid > 1 && !exists('/proc/' + lock.pid), 'PREVIOUS_CONTROLLER_NOT_CONFIRMED_STOPPED');
  checkPreviousFailure(old.readJson(FAILED_RECORD + '/worker-output.json'));
  const previousUnit = old.readJson(FAILED_RECORD + '/worker-unit.json');
  ensure(previousUnit.unit === FAILED_UNIT && previousUnit.label === '1d2b8fc0e133', 'PREVIOUS_UNIT_CHANGED');
  const state = observeWorker(FAILED_UNIT); old.checkInactiveUnit(state.exit, state.values, state.processes);
  ensure(old.hash(FAILED_AREA + '/build/Containerfile') === SOURCE_HASHES['runner/Containerfile'] && !exists(FAILED_AREA + '/build/image.id') && !exists(FAILED_RECORD + '/result.json'), 'PREVIOUS_BUILD_FILES_CHANGED');
  const checked = old.readJson(FAILED_RECORD + '/website-postcheck.json'); ensure(checked.unchanged === true && checked.runEnabled === false, 'PREVIOUS_WEBSITE_CHECK_MISSING');
  save('previous-failure.json', { record: FAILED_RECORD, state, lockRecord: lock.record, originalScriptSha256: ORIGINAL_SCRIPT_SHA });
  log('旧构建失败原因和任务结束状态已核对，原目录、记录和锁文件全部保留。');
}
...
async function runWorker(label) {
    ...
    child.stdout.on('data', bytes => {
      ...
      for (;;) {
        ...
        try {
          ...
          if (row.event === 'context-ready') log('新构建副本已将 WORKDIR 提前；原文件和所有权限限制保持。');
          if (row.event === 'build-result' && (row.result?.code !== 0 || row.result?.reason || row.result?.signal)) {
            log('构建 stdout（脱敏摘要）：\n' + diagnosticHelpers.redact(row.result?.stdout, 4000));
            log('构建 stderr（脱敏摘要）：\n' + diagnosticHelpers.redact(row.result?.stderr, 4000));
          }
          if (row.event === 'case-result') {
            ...
            if (expected && row.result?.state !== expected.state) log('编译/运行错误（脱敏摘要）：\n' + diagnosticHelpers.redact((row.result?.compiler_output || '') + '\n' + (row.result?.stderr || '') + '\n' + (row.result?.message || ''), 4000));
          }
  ...
  ensure(completed.containerfileSha256 === createHash('sha256').update(buildContainerfile(checkSource(ROOT + '/runner/Containerfile', SOURCE_HASHES['runner/Containerfile']))).digest('hex'), 'BUILT_CONTEXT_HASH_MISMATCH');
...
async function main() {
  ...
  const logPath = ROOT + '/logs/compiler-check-02-' + randomBytes(6).toString('hex') + '.log';
  logFd = fs.openSync(logPath, 'wx', 0o600); log('操作日志：' + logPath);
  record = fs.mkdtempSync(ROOT + '/backups/compiler-check-02-'); log('私有操作记录：' + record);
  step('只读接续预检：旧构建失败记录、现有执行代码、网站与资源限额'); await loadChecks();
  previousFailure();
  ...
  save('result.json', { stage: 'teaching-image-and-containers-verified', revision: '02', at: new Date().toISOString(), importRecord: IMPORT_RECORD, previousFailure: FAILED_RECORD, ...completed, sourceHashes: SOURCE_HASHES, pm2 });
...
if (process.argv[2] === '--resume-compiler-containers-02') {
  try { await main(); }
