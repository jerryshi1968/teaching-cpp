# 本轮修正版：修复数据库检查后启用 C++ 编辑

> 本步骤已成功执行，以下命令仅保留历史，不再运行。C++ 新建、保存和重新打开已由用户确认正常；执行仍关闭。实际日志为 write-enable-02-crzIeNIg.log，配置备份为 backups/write-enable-0uHa5u。

旧 01 包已在数据库预检中报 ER_UNSUPPORTED_PS；未改配置、未重启服务。本页为新的 02 修正版，服务器尚未执行，不要重跑旧包。上传下面两个本地文件到 `/var/www/teaching-cpp-backend`，使用 WinSCP 二进制传输，不上传整个项目：

- `G:\teaching-cpp\releases\teaching-cpp-write-enable-20260831-02.tar.gz`
- `G:\teaching-cpp\releases\teaching-cpp-write-enable-20260831-02.tar.gz.sha256`
...
(
  ...
  cpp_archive=teaching-cpp-write-enable-20260831-02.tar.gz
  cpp_sha=c919d456b448fe800eaa967fac44827dd56a6c0dd8710ea02abe4bcba32a833a
  printf '%s  %s\n' "$cpp_sha" "$cpp_archive" | sha256sum -c -
  sha256sum -c "$cpp_archive.sha256"

  cpp_step=deploy/write-enable-20260831-02
  ...
  test ! -L "$cpp_lock"
  cpp_log=$(mktemp /var/www/teaching-cpp-backend/logs/write-enable-02-XXXXXXXX.log)
...
完整范围与恢复说明见 [写入启用说明](../deploy/write-enable-20260831-02/README.md)。本地检查为 109 项通过、1 项可选 MySQL 跳过；服务器上的实际启用和浏览器验收仍待回报。
