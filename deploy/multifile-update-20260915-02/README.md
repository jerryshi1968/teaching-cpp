...
```powershell
& 'D:\Program Files\nodejs\node.exe' deploy\multifile-update-20260915-02\package.mjs
```
...
将压缩包和外部 `.sha256` 上传到 `/var/www/teaching-cpp-backend`，核对后解压。包内所有内容都位于 `deploy/multifile-update-20260915-02/`。

```bash
cd /var/www/teaching-cpp-backend
sha256sum -c teaching-cpp-multifile-update-20260915-02.tar.gz.sha256
test ! -e deploy/multifile-update-20260915-02
tar -xzf teaching-cpp-multifile-update-20260915-02.tar.gz
tools/node/bin/node deploy/multifile-update-20260915-02/update.mjs --check-only
```
...
```bash
tools/node/bin/node deploy/multifile-update-20260915-02/update.mjs --preflight
```
...
```bash
tools/node/bin/node deploy/multifile-update-20260915-02/update.mjs --publish --backup-confirmed --production-reviewed
```
