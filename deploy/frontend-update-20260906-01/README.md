# C++ 前端更新：teaching-prj-mgmt v0.1.1

本发布材料只更新 `https://tigao123.com/teaching-cpp/` 的静态前端，来源提交固定为 `9d162431b8307bf4708c9248ecf88ce4584b5ffc`。它不迁移或连接数据库，不修改 `.env`、Apache 配置、主站 `.htaccess`、PM2 配置或 systemd 配置，不重启或平滑加载任何服务，也不操作 C++ 执行容器。
...
- 新前端升级到 `teaching-prj-mgmt v0.1.1`，让多列网格拖放按指针位置排序，支持把项目拖到有效面包屑移至上级或根目录，并显示拖放预览及有效目标高亮。
...
- 当前公开前端必须与已成功发布的 `frontend-update-20260905-08` 四个构建文件完全一致。不同则只读预检停止，不猜测覆盖。
...
- `frontend-update-20260905-08` 已成功发布并通过公开检查，精确对齐了操作图标、按钮盒子和底栏分隔线；本版本升级共享组件并修正网格排序及面包屑拖放。
...
```powershell
& 'D:\Program Files\nodejs\node.exe' deploy\frontend-update-20260906-01\package.mjs
```
...
sha256sum -c teaching-cpp-frontend-update-20260906-01.tar.gz.sha256
test ! -e deploy/frontend-update-20260906-01
tar -tzf teaching-cpp-frontend-update-20260906-01.tar.gz
tar -xzf teaching-cpp-frontend-update-20260906-01.tar.gz
/var/www/teaching-cpp-backend/tools/node/bin/node deploy/frontend-update-20260906-01/update.mjs --check-only
```

压缩包内容必须全部位于 `deploy/frontend-update-20260906-01/`。`--check-only` 只核对包内清单、构建文件和可复现依赖，不读取生产配置或连接服务。
...
```bash
/var/www/teaching-cpp-backend/tools/node/bin/node deploy/frontend-update-20260906-01/update.mjs --preflight
```
