# C++ 前端更新：teaching-prj-mgmt v0.1.0

本发布材料只更新 `https://tigao123.com/teaching-cpp/` 的静态前端，来源提交固定为 `b7763ad6e0753b5a442bd6940c8829eeaaebcd56`。它不迁移或连接数据库，不修改 `.env`、Apache 配置、主站 `.htaccess`、PM2 配置或 systemd 配置，不重启或平滑加载任何服务，也不操作 C++ 执行容器。
...
- 新前端包含与 p5.js 作品管理页布局一致的全宽 `ProjectOrganizer` 作品工坊，并把作品与作品组卡片的拖动、分发、上下排序、跨组移动、重命名和删除按钮对齐为 p5.js 的图标、顺序、尺寸、间距及状态样式。
...
- 当前公开前端必须与已成功发布的 `frontend-update-20260905-05` 四个构建文件完全一致。不同则只读预检停止，不猜测覆盖。
...
- `frontend-update-20260905-05` 已成功发布并通过公开检查，继续对齐了 p5.js 的班级选择、标题、按钮和作品卡片布局；本版本只修正卡片操作按钮的一致性。
...
```powershell
& 'D:\Program Files\nodejs\node.exe' deploy\frontend-update-20260905-06\package.mjs
```
...
sha256sum -c teaching-cpp-frontend-update-20260905-06.tar.gz.sha256
test ! -e deploy/frontend-update-20260905-06
tar -tzf teaching-cpp-frontend-update-20260905-06.tar.gz
tar -xzf teaching-cpp-frontend-update-20260905-06.tar.gz
/var/www/teaching-cpp-backend/tools/node/bin/node deploy/frontend-update-20260905-06/update.mjs --check-only
```

压缩包内容必须全部位于 `deploy/frontend-update-20260905-06/`。`--check-only` 只核对包内清单、构建文件和可复现依赖，不读取生产配置或连接服务。
...
```bash
/var/www/teaching-cpp-backend/tools/node/bin/node deploy/frontend-update-20260905-06/update.mjs --preflight
```
